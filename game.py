...
class Game:
    ...
    def __init__(self):
        ...
        level1 = os.path.join("Jeux", "LVL 1", "00Map LVL 1.canvas")
    ...
    def setup(self):
        ...
        self.commands["look"]= Command(
            "look",": savoir ce qu'il y a dans l'inventaire",Actions.look,0
        )
        self.commands["take"]= Command(
            "take",": prendre un item (take all pour tout prendre)",Actions.take,1
        )
        self.commands["drop"]= Command("drop",": déposer un item dans la salle",Actions.drop,1)
